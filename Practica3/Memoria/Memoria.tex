...
En el conjunto de datos al que aplicamos \emph{PCA}, probamos las transformaciones polinímicas en el conjunto $\{1, 2, 3, 4\}$. Al conjunto de datos al que no aplicamos \emph{PCA}, no aplicamos transformaciones polinómicas, pues nuestro ordenador no es capaz de calcular estas transformaciones.
...
La tabla deja claro que es mejor no aplicar \emph{PCA} y transformaciones polinómicas. Quizás con otra transformación sí que sería útil aplicar \emph{PCA}, pero en este caso no hemos encontrado unas trasnformaciones útiles. Centrándonos en las tres variantes para el \emph{dataset} sin aplicar \emph{PCA}, el mejor modelo es \emph{Linear Regression}. Sin embargo, \emph{Ridge} pierde en media por un error cuadrático medio de tan solo $0.27$. Como todavía no hemos encontrado un valor de $\lambda$ óptimo, elegimos este modelo a pesar de que tengamos un error ligeramente mayor, con la esperanza de que en la segunda fase de \emph{Cross Validation} consigamos un error menor.
...
$. Tenemos, tras toda la limpieza de los datos, algo más de 15.500 columnas, y por tanto, al trabajar con modelos lineales, podemos trabajar con un número de variables en el orden de $1500$ columnas. Es claro que con 81 columnas no nos interesa hacer que algunas columnas sean cero, que es lo que consigue \emph{Lasso}. Lo que sí nos interesa es que los valores de los parámetros del modelo no sean muy grandes, que es lo que consigue \emph{Ridge}.

\subsubsection{\emph{Cross-Validation, segunda etapa}}

Como ya hemos justificado, elegimos regresión \emph{Ridge} sobre el conjunto de datos al que no aplicamos \emph{PCA}. Recordar que, a pesar de no estar aplicando \emph{PCA}, si estamos aplicando el borrado de \emph{outliers} y la estandarización de los datos.
...
Los resultados de esta tabla eran previsibles por lo que ya hemos comentado. El error aumenta monótamente salvo en el salto de $\lambda = 0.001$ a $\lambda = 0.01$. El mejor resultado lo obtenemos con $\lambda \in \{10^{-7}, 10^{-6}\}$. Elegimos por tanto $10^{-6}$, con $10^{-7}$ puede ser que estemos castigando muy poco en la parte de regularización.

\pagebreak
\subsection{Entrenamiento sobre todo el \emph{train\_dataset} para seleccionar el modelo final}

En este punto está justificado el que usemos el conjunto original de datos, al que hemos eliminado \emph{outliers} y estandarizado, sin emplear una técnica como \emph{PCA}. Además, usaremos como parámetros:
...
Es claro que no hemos tenido problemas de \emph{overfitting}, pues el error en test no es demasiado dispar al error en la muestra. Los coeficientes de correlación son buenos, estamos explicando el $73\%$ de la varianza de la muestra con nuestro modelo lineal.

El error absoluto medio en el test se puede interpretar como que de media, para cada dato estamos prediciendo con un error $\pm 13.4 K$. Respecto a la bondad de nuestros resultados, en el paper original \cite{original_paper_reg:paper} se comenta que obtienen resultados con error \emph{rmse} de $9.5K$, donde \emph{rmse} es la raíz del error cuadrático medio. Nosotros obtenemos un \emph{rmse} en test de $17.64K$. Por tanto, con un modelo mucho más simple que el empleado en \cite{original_paper_reg:paper}, basado en árboles, hemos obtenido un \emph{rmse} de menos del doble.

Además, la incertidumbre $\pm 13.4 K$ en datos que de media tiene un $T_c = 34.37$ y que se mueve en un rango $T_c \in [0.00, 185.00]$ parece aceptable, aunque claramente muy mejorable.

El principal problema con el que nos hemos encontrado ha sido realizar una reducción de la dimensionalidad de nuestro conjunto de datos, acompañada con una transformación no lineal de los datos, que provocase un aumento del performance de la función aprendida sin caer en problemas de generalización. Como se ha justificado durante las secciones anteriores, \emph{Cross Validation} ha mostrado que nuestra transformación de los datos no ha sido efectiva, usando prácticamente los datos tal y como se nos han dado (previo borrado de \emph{outliers} y estandarización obligada).

Por tanto, la principal mejora al trabajo presentado sería explorar más el problema para lograr encontrar, gracias a conocimiento experto sobre el problema, una \emph{NLT} efectiva. También sería interesante probar con modelos más avanzados para ver sí somos capaces de alcanzar resultados al nivel de los presentados en \cite{original_paper_reg:paper}.
