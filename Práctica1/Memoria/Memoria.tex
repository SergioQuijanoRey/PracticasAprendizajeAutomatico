...
\begin{itemize}
    \item starting\_point: punto inicial del que parte la búsqueda
    \item loss\_function: función de error que buscamos minimizar. En nuestro caso concreto, es una función real de dos variables
    \item gradient: función vectorial 2-dimensional que representa el gradiente de loss\_function
    \item learning\_rate: la tasa de aprendizaje. Es el parámetro crítico, pues en esta sección nos centramos en estudiar el comportamiento del gradiente descendente en base a este parámetro (y también del starting\_point)
    \item max\_iterations: número máximo de iteraciones. Así tenemos una condición sufiente para saber que el algoritmo va a parar
    \item target\_error: error debajo del cual paramos de iterar
    \item verbose: si es True, la función devuelve el error de cada solución de las iteraciones
\end{itemize}
...
\begin{code}
La primera iteracion en la que el error esta por debajo de 10e-14 es: 9 (contando desde cero)
Las primeras coordenadas que estan por debajo de ese error: [1.15728875 0.9108384 ]
\end{code}
...
El punto con una cruz blanca es el punto inicial. El punto con una cruz negra es el punto final de la búsqueda. En este caso podríamos seguir avanzando con la búsqueda, pero decidimos parar porque estamos por debajo de la cota de error pedida.

Se manda explícitamente que usemos flotantes de 64bits, para lo cual usamos la orden np.float64 para devolver todas nuestras funciones que representan los errores y las derivadas parciales.
